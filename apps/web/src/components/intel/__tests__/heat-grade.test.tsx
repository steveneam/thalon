...
describe("heat grading (founder direction 2026-07-07)", () => {
  ...
  it("renders bar length + grade word, with the exact score demoted to tooltip/aria", () => {
    ...
    expect(grade).toHaveTextContent("rising");
    // Length is the primary encoding — the fill is 66% of the track.
    expect(grade.querySelector(".bg-heat-3")).toHaveStyle({ width: "66%" });
  ...
  it("clamps out-of-range scores instead of overflowing the track", () => {
    ...
    expect(grade.querySelector(".bg-heat-4")).toHaveStyle({ width: "100%" });
  });
});
