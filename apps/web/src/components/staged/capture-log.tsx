    ...
    <section aria-label="Captured edits" className="flex flex-col gap-1.5 rounded-lg border border-border p-2">
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs text-muted-foreground">
          {captures.length} {captures.length === 1 ? "interaction" : "interactions"} captured as edit_diff rows
          (RFC-6902, verbatim) — advanced-mode telemetry trains one-prompt defaults.
        </p>
        {captures.length > 0 && (
        ...
        <ul className="flex flex-col gap-1">
          {recent.map((capture) => (
            <li key={capture.id} className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
              <Badge variant="secondary" className="font-mono text-[10px]">{capture.kind}</Badge>
              <span className="font-mono text-[10px]">{capture.stageKey}</span>
