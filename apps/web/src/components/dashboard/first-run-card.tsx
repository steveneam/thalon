    ...
    <Card className="border-primary/40 bg-primary/5">
      ...
      <CardContent className="flex flex-col gap-3">
        <ol className="grid gap-2 text-sm sm:grid-cols-3">
          {(
            [
              { step: "1 · profile", copy: "Tell Thalon who it writes for — company, voice, topics.", href: "/app/profiles" },
              { step: "2 · prompt", copy: "Feel the staged video flow on the built-in demo — no keys, no spend.", href: "/app/create" },
              { step: "3 · approve", copy: "Nothing ships without your click — walk the queue once.", href: "/app/approve" },
            ] as const
          ).map(({ step, copy, href }) => (
            <li key={step}>
              <Link
