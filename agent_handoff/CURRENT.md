...
▎ ▸ **Session-28 plan:** (1) **B7.a cadence gate** — judge-harness rule beside denylist/grounding consuming `cadence` config (counting queued/approved drafts per platform per window — derivable from existing tables, no new schema); (2) **B7.e routing table** — bucket→platform map consumed at fan-out; both on the `features` lane branch `agent/features/b7ae` (sequential lead work); (3) **dogfood the leads queue on staging** — seed tenant #0's ICP (verbatim from proposal §ICP seed draft, founder edits pending) via the profiles POST, import a real prospect CSV, first real ranked queue → founder review — **the dogfood triage rows ARE B-crm.5's training corpus** (eval rows are append-only; the learn loop consumes the backlog retroactively at the checkpoint); (4) if room: B7.c persona-brief editor (apps/web is free — leads lane merged). **B-crm.5 stays at the checkpoint per A16 (founder ratified the lead's recommendation post-wrap, 2026-07-13): building the tuner before dogfood data exists would tune on an empty corpus, and the two-axis grading decision (research brief §5 rec 2) may reshape the weights first — present at the checkpoint with real triage data + the algorithm shortlist.** **Nothing in this plan needs the vendor** — the meta-prompt + Higgsfield tier gate ONLY the deferred visual block (B7.2/7.3/7.4).
