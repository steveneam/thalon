...
▎ ▸ **Session-28 plan:** (1) **B7.a cadence gate** — judge-harness rule beside denylist/grounding consuming `cadence` config (counting queued/approved drafts per platform per window — derivable from existing tables, no new schema); (2) **B7.e routing table** — bucket→platform map consumed at fan-out; both on the `features` lane branch `agent/features/b7ae` (sequential lead work); (3) **B-crm.5 no-dependency slice — FOUNDER PULL-FORWARD (post-wrap message, 2026-07-13): Beta-posterior learn loop + Wilson lower bound consuming the existing `lead_triage` eval rows (weight suggestions proposed via config, applied auditable — never silently) + HN-gravity urgency sort as a queue-order option** (research brief §5 recs 1+5; record as a charter note at session start — A16 had queued B-crm.5 for the checkpoint); (4) **dogfood the leads queue on staging** — seed tenant #0's ICP (verbatim from proposal §ICP seed draft, founder edits pending) via the profiles POST, import a real prospect CSV, first real ranked queue → founder review; (5) if room: B7.c persona-brief editor (apps/web is free — leads lane merged). **Nothing in this plan needs the vendor** — the meta-prompt + Higgsfield tier gate ONLY the deferred visual block (B7.2/7.3/7.4).
