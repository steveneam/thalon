...
2b. **W-audit item (a) is founder-confirmed FIRST audit work: register the concept film through the product's own pipeline (pin masters+keepers into the object store, write video_projects/takes/cuts rows) so Videos shows the flagship film.** Checkpoint candidate riding row 4: **B-sitegen** (the ENGINE GAP, founder q s60: meta-prompt behind Create's page family — category/header fills slots → build loop → judge → approve). **Storage decisions RECORDED s60: server-side system-of-record (client machines only get export copies) · VPS-local object stores NOW, AWS/S3 migration parked until real traffic/customers · storage-story audit = W-audit item (e).**
