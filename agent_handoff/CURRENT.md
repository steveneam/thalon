...
2. **Main line (as verdicts allow):** Phase R = the s40 consistency re-critique against the redesigned workspace (founder GO first) · the Phase-I contract window (planned-slot store + scheduledFor · lead stage field · captures list read · capture-id on drafts · saved-views store — use `/contract-window`, founder approval) — these two can run as parallel lanes with fresh approval.
