...
- **Render-speed ratchet candidate (founder ask):** keep hyperframes — the slowness is the Windows screenshot fallback (BeginFrame = Linux-only) + load; **Docker is already installed on the box**, so a `render:docker` Linux-container wrapper is the cheap multi-× speedup (ADR-0004 §7's own path) — slot at the next tooling window; render-farm scale rides the server-side briefing. Lane-prep lesson adopted: pre-provision machine runtimes BEFORE cutting lanes (kickoffs list required runtimes; lead installs + doctor-verifies at prep).
