...
- **Amendment (B-ve.6 chartered, founder-directed s50 2026-07-17):** one view-layer bucket appended to the window — the NLE **track view** ("music/video chunks against a time axis", founder ask; design brief = `docs/research/nle-timeline-ui-patterns.md`, researched same-session at founder direction: FCP/Premiere/Resolve/CapCut/web editors + Resolve-20 AI-integration addendum). The EDL chain renders as a MAGNETIC timeline (FCP model — the contract's own physics): drag = ripple reorder, edge-drag = trim, caption chips/music block/endcard freeze draggable at absolute times, ruler + playhead scrub + snapping + Esc-cancel; every gesture funnels through the existing B-ve.3 pure transforms — zero contract change, no new mutation paths. Editor page adopts the NLE geometry (preview + inspector row, FULL-WIDTH timeline band) + a Project/Editor tab rail. Sequencing verdict: track view FIRST, wave-2 mint slips one session. Named follow-up candidate: **B-ve.7 agent reframe** — crop/pan ops joining the edlDiff alphabet through the existing propose→judge→approve door (Resolve Smart-Reframe equivalent, own-engine, 0cr).
