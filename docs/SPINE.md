...
- `judge.candidate.g3_screen` · `judge.candidate.g3_final` — the candidate judge (s89): the SAME gate ladder (`proprietary/judge/src/gate-ladder.ts`, one ladder for both entry points) graded against a body that is not yet the draft's, so the R8 AI edit proposes only what the judge passed. The verdict is advisory (`ofRecord: false`) — no `judge_results` row, no transition, because a verdict about text not on the draft has no honest `body_hash` to bind to — but the spend is real and metered under its own label
...
- `intel.dossier` — trend-card dossier (titles/angles/hook); G1 denylist gates the whole dossier before it rides the sweep wire; per-sweep ration `TREND_DOSSIER_CARDS`, default disarmed (B6.5 half-step)
- `outreach.compose_email` — draft-only outreach email from an operator-pruned lead brief; the brief is also the grounding source, the full judge gate (G1 + cadence on `email` + G3×2) runs downstream, and NO send path exists anywhere (B-crm.4 front half, s29)
- `video.propose_edl_diff` — agent-proposed EDL diff over a cut (measured ops only: caption-move · caption-text · music-align, each carrying its `why`); the core zod-validates AND dry-applies the candidate, the operator approves per-op, and an applied diff rides the SAME save door as a manual edit — replay-verified (base + diff must reproduce the submitted EDL) and attributed (model + prompt hash + the exact diff). Rejections become eval rows (B-ve.4)
- `create.describe_reference` — the reference-describe vision call: an operator's `reference`-role media becomes style/subject TEXT that threads into the family prompt, so inspiration can inform generation while its bytes stay behind the licensing wall (spec `docs/create-engine/spec.md` §Design/The engine). **The only call in this repo handed an image**, and therefore the only one on its own `MODEL_VISION` tier — the draft tier's shipped model is text-only, so reusing it would ship a default that cannot work. Scope is deliberately narrow: STORED bytes only, read content-address-verified; an EXTERNAL ref is never fetched and an AUDIO ref is refused in words, both **before** the guard, so an undescribable reference costs neither a budget assertion nor a call and degrades to the honest "reference attached, not yet analysed — <reason>" (B-create.2)
- `create.ai_edit` — the R8 AI edit: an operator instruction rewrites ONE variant's body. **Proposes only, and only what the judge passed** (s89) — the rewrite is graded by the candidate judge before it can become a proposal, so a judge-refused rewrite returns the reason verbatim and the operator's known-good text is never touched (the spec's Error Behavior, met exactly); core writes nothing, so the draft's body and `body_hash` are untouched on every path; the operator applies through the existing `approvals.record({action:"edit"})` door (which is what writes `edit_diffs` + `eval_cases` + the body swap), and the shared judge harness re-gates before the variant can leave the Composer — the double judge is deliberate, because only the post-land verdict binds to the hash I1 checks. A rewrite that comes back identical is the prompt file's own refusal convention and is surfaced as a refusal rather than landed as a no-op edit (B-create.2)
