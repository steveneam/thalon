...
- `create.ai_edit` — the R8 AI edit: an operator instruction rewrites ONE variant's body. **Proposes only** — core writes nothing, so the draft's body and `body_hash` are untouched on every path; the operator applies through the existing `approvals.record({action:"edit"})` door (which is what writes `edit_diffs` + `eval_cases` + the body swap), and the shared judge harness re-gates before the variant can leave the Composer. A rewrite that comes back identical is the prompt file's own refusal convention and is surfaced as a refusal rather than landed as a no-op edit (B-create.2)
