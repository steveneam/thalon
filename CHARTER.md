...
Standing dogfood flywheel: origination runs across all three tenants → eval rows; founder drops viral/trending posts as exemplars (manual, ToS-clean). **Pull-trigger B3.12** — if manual exemplar volume runs short, charter a lean official-API trend intake (e.g. YouTube Data API trending/search metadata → exemplar candidates); acquisition stays official-APIs-only per A5. Parallel [you] track: LinkedIn + X OAuth developer apps (B3.1 long pole) · gateway credit top-up (judge tier needed this sprint) · profile content for the two real companies.
